...
async function handleSocialAuth(provider, options) {
    ...
    options.socialProvider = provider;
    var port = await startCallbackServer(codeVerifier, state, options);
    var authUrl = OAUTH_CONFIG.authServiceEndpoint + '/login?idp=' + provider + '&redirect_uri=' + encodeURIComponent('kiro://kiro.kiroAgent/authenticate-success') + '&code_challenge=' + codeChallenge + '&code_challenge_method=S256&state=' + state;
    return { authUrl: authUrl, authInfo: { provider: 'claude-kiro-oauth', authMethod: 'social', socialProvider: provider, port: port, state: state } };
}
...
async function handleBuilderIDDeviceCode(options) {
    ...
    var regResp = await axios.post(ssoEndpoint + '/client/register', { clientName: 'Kiro IDE', clientType: 'public', scopes: OAUTH_CONFIG.scopes }, { headers: { 'Content-Type': 'application/json', 'User-Agent': 'KiroIDE' }, timeout: 30000 });
    var authResp = await axios.post(ssoEndpoint + '/device_authorization', { clientId: regResp.data.clientId, clientSecret: regResp.data.clientSecret, startUrl: startURL }, { headers: { 'Content-Type': 'application/json' }, timeout: 30000 });
    
    var deviceAuth = authResp.data;
    var taskId = 'kiro-' + deviceAuth.deviceCode.substring(0, 8) + '-' + Date.now();
    pollDeviceToken(regResp.data.clientId, regResp.data.clientSecret, deviceAuth.deviceCode, deviceAuth.interval || 5, deviceAuth.expiresIn || 300, taskId, { region: region }).catch(function(e) { logger.error('[Kiro Auth] Polling failed: ' + e.message); });

    return { authUrl: deviceAuth.verificationUriComplete, authInfo: { provider: 'claude-kiro-oauth', authMethod: 'builder-id', deviceCode: deviceAuth.deviceCode, userCode: deviceAuth.userCode, verificationUri: deviceAuth.verificationUri, verificationUriComplete: deviceAuth.verificationUriComplete, expiresIn: deviceAuth.expiresIn, interval: deviceAuth.interval } };
}
...
async function pollDeviceToken(clientId, clientSecret, deviceCode, interval, expiresIn, taskId, options) {
    ...
    var poll = async function() {
        ...
        try {
            var resp = await axios.post(ssoEndpoint + '/token', { clientId: clientId, clientSecret: clientSecret, deviceCode: deviceCode, grantType: 'urn:ietf:params:oauth:grant-type:device_code' }, { headers: { 'Content-Type': 'application/json', 'User-Agent': 'KiroIDE' }, timeout: 30000 });
            if (resp.data && resp.data.accessToken) {
                logger.info('[Kiro Auth] Token obtained [' + taskId + ']');
                var tokenData = { accessToken: resp.data.accessToken, refreshToken: resp.data.refreshToken, expiresAt: new Date(Date.now() + (resp.data.expiresIn || 3600) * 1000).toISOString(), authMethod: 'builder-id', clientId: clientId, clientSecret: clientSecret, idcRegion: region };
                var credPath = await saveCredentials(tokenData);
                var pool = getAccountPool();
                await pool.addAccountFromFile(credPath);
...
async function startCallbackServer(codeVerifier, state, options) {
    for (var port = OAUTH_CONFIG.callbackPortStart; port <= OAUTH_CONFIG.callbackPortEnd; port++) {
        try {
            var server = await createCallbackServer(port, codeVerifier, state, options);
            activeServers.set(port, server);
            logger.info('[Kiro Auth] Callback server on port ' + port);
            return port;
        } catch (e) { if (e.code !== 'EADDRINUSE') logger.warn('[Kiro Auth] Port ' + port + ' error: ' + e.message); }
    }
    ...
    return new Promise(function(resolve, reject) {
        var server = http.createServer(async function(req, res) {
            try {
                ...
                if (error) { res.writeHead(400, { 'Content-Type': 'text/html; charset=utf-8' }); res.end('Auth failed: ' + error); return; }
                if (state !== expectedState) { res.writeHead(400, { 'Content-Type': 'text/html; charset=utf-8' }); res.end('State mismatch'); return; }

                var tokenResp = await axios.post(OAUTH_CONFIG.authServiceEndpoint + '/oauth/token', { code: code, code_verifier: codeVerifier, redirect_uri: 'kiro://kiro.kiroAgent/authenticate-success' }, { headers: { 'Content-Type': 'application/json' }, timeout: 30000 });
                var tokenData = { accessToken: tokenResp.data.accessToken, refreshToken: tokenResp.data.refreshToken, profileArn: tokenResp.data.profileArn, socialProvider: options.socialProvider, expiresAt: new Date(Date.now() + (tokenResp.data.expiresIn || 3600) * 1000).toISOString(), authMethod: 'social', region: 'us-east-1' };
                ...
                res.end('<script>window.close()</script>Success! You can close this tab.');
        ...
        setTimeout(function() { if (server.listening) { server.close(); activeServers.delete(port); } }, OAUTH_CONFIG.authTimeout);
    });
}
...
async function refreshKiroToken(refreshToken, region) {
    ...
    return { accessToken: resp.data.accessToken, refreshToken: resp.data.refreshToken || refreshToken, profileArn: resp.data.profileArn || '', expiresAt: new Date(Date.now() + (resp.data.expiresIn || 3600) * 1000).toISOString(), authMethod: 'social', region: region || 'us-east-1' };
}
